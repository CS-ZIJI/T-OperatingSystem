...
!!! info "导读"
    ...
    在[存储](./5-x.md){target="_blank"}一章，我们提到过，卷(volume)做初始化时，需要建立一个文件系统。文件系统提供了数据存储形式的逻辑视图，将数据以**文件(file)**的形式从硬件存储中抽象出来，并使用**目录(directory)**对文件进行进行结构化的组织和管理。
...
<figure markdown>
<center> ![](imgs/52.png){ width=80% } </center>
...
<figure markdown>
<center> ![](imgs/53.png){ width=80% } </center>
...
<figure markdown>
<center> ![](imgs/54.png){ width=80% } </center>
...
<figure markdown>
<center> ![](imgs/55.png){ width=80% } </center>
...
<figure markdown>
<center> ![](imgs/56.png){ width=80% } </center>
...
<figure markdown>
<center> ![](imgs/57.png){ width=80% } </center>
...
2. Basic file system
    ...
    - 与 [I/O 调度](./5-x.md#调度){target="_blank"}有关；
...
我们在讲内存的时候，讨论过[进程的内存分配策略](./3-1-x.md#连续分配及其问题){target="_blank"}。这里讨论的内容是类似的，我们考虑文件内存要如何被分配，同时，块如何分配也暗示了块是如何组织的。
...
???+ section "continuous"
    ...
    <figure markdown>
    <center> ![](imgs/60.png){ width=80% } </center>
    ...
    !!! not-advice "disadvantages"
        ...
        - 分配时寻找合适空间比较困难；
            - 类似[进程的内存分配策略](./3-1-x.md#连续分配及其问题){target="_blank"}，需要考虑外部碎片等问题；
...
???+ section "linked"
    ...
    <figure markdown>
    <center> ![](imgs/61.png){ width=80% } </center>
...
???+ section "index"
    ...
    <figure markdown>
    <center> ![](imgs/62.png){ width=80% } </center>
    ...
    1. 使用链接索引，用链表将它们连起来；
    2. 使用多级索引（回顾[多级页表](3-1-x.md#页表设计改进){target="_blank"}）；
    ...
    <figure markdown>
    <center> ![](imgs/63.png){ width=80% } </center>
    The UNIX inode.
    </figure>
...
<figure markdown>
<center> ![](imgs/59.png){ width=80% } </center>
